...
5. **Map tiles**: three transport modes. `--tile-mode=auto` (default) tries in order: local `.pmtiles` if present → Protomaps online API if `--protomaps-api-key` / `$PROTOMAPS_API_KEY` is set → OpenFreeMap (zero-config, no key, hosted positron style). Explicit modes (`pmtiles`, `online`, `openfreemap`) short-circuit the chain. All three code paths must keep working. Detail + rationale in [`../research/online-tile-fallback.md`](../research/online-tile-fallback.md). *(Pre-2026-05-17 wording was "Protomaps `.pmtiles` always, dev and prod" — first revised when the 8.7 GB archive turned out to be a punishing onboarding tax, then revised again to add OpenFreeMap so the binary always renders something with zero configuration.)*
